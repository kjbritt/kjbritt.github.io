---
layout: post
title:  "UAA undergrads study wolf behaviors with ADF&G"
author: "Keenan J. Britt"
description: "UAA students are used to ‘howling’ for the Seawolves, but last semester a small group of undergraduates had the opportunity to study Southcentral Alaska’s wild wolves thanks to a partnership with the Alaska Department of Fish and Game."
image: /docs/assets/images/carpenter_site_views.jpg
date:   2025-03-18 01:06:33 +0000
category: wildlife
---
{% include breadcrumbs.html %}

UAA students are used to ‘howling’ for the Seawolves, but last semester a small group of undergraduates had the opportunity to study Southcentral Alaska’s wild wolves thanks to a partnership with the Alaska Department of Fish and Game.

Full story available online via UAA Green & Gold News. 

<button onclick="newTab()" style="background-color: #7f7053; color: white; padding: 15px 32px; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">Full story</button>
<script>
function newTab() {
  window.open("https://www.uaa.alaska.edu/news/archive/2025/03/wolf-study-adfg.cshtml");
}
</script>