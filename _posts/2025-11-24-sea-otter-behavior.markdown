---
layout: post
title:  "Diving into the data: UAA grad student analyzes sea otter behavior"
author: "Keenan J. Britt"
description: "“We are fortunate to know a lot about sea otter behavior, but there’s also a lot that we don’t know,” said Ana Velasquez, a master’s candidate in biological sciences at UAA. For her thesis project, Velasquez used old data to make new discoveries about otters."
image: /docs/assets/images/carpenter_site_views.jpg
date:   2025-11-24 01:06:33 +0000
category: wildlife
tags: wildlife oceans
---
{% include breadcrumbs.html %}

“We are fortunate to know a lot about sea otter behavior, but there’s also a lot that we don’t know,” said Ana Velasquez, a master’s candidate in biological sciences at UAA. For her thesis project, Velasquez used old data to make new discoveries about otters.

Full story available online via UAA Green & Gold News. 

<button onclick="newTab()" style="background-color: #7f7053; color: white; padding: 15px 32px; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">Full story</button>
<script>
function newTab() {
  window.open("https://www.uaa.alaska.edu/news/archive/2025/11/diving-into-the-data.cshtml");
}
</script>
