---
layout: post
title:  "UAA grad student investigates the connection: volcanoes, lake trout and public health"
author: "Keenan J. Britt"
description: "Residents of Southcentral Alaska have been preparing for the possible eruption of Mt. Spurr over the last few months. Recent UAA alumnus Paul Gabriel is focused on another volcanic public health concern: mercury."
image: /docs/assets/images/carpenter_site_views.jpg
date:   2025-05-22 01:06:33 +0000
category: wildlife
---
{% include breadcrumbs.html %}

Residents of Southcentral Alaska have been preparing for the possible eruption of Mt. Spurr over the last few months. Recent UAA alumnus Paul Gabriel is focused on another volcanic public health concern: mercury.

Full story available online via UAA Green & Gold News. 

<button onclick="newTab()" style="background-color: #7f7053; color: white; padding: 15px 32px; border: none; border-radius: 8px; cursor: pointer; font-size: 16px;">Full story</button>
<script>
function newTab() {
  window.open("https://www.uaa.alaska.edu/news/archive/2025/05/paul-gabriel-volcanic-mercury.cshtml");
}
</script>